...
public class WaitForWebhookTest {
    ...
    public void testWaitHook() throws Exception {
        ...
        // Tokens with the same custom token tend to overwrite themselves and give a 202 return instead of 200. Non
        // deterministic behavior warning.
        p.setDefinition(new CpsFlowDefinition(pipelineCode, true));
    ...
    public void testWaitAuthHook_sendNoAuthToken() throws Exception {
        ...
        // Use a WebClient to send a json file to trigger the webhook
        final FailingHttpStatusCodeException ex = assertThrows(
                FailingHttpStatusCodeException.class, () -> trigger_webhook("webhook-step/" + webHook_ID, content));

        // TODO: in certain cases, this assert fails with "expected org.htmlunit.FailingHttpStatusCodeException to be
        // thrown, but nothing was thrown"
        assertThat(
                "Triggering the webhook with wrong authentication should fail", ex.getStatusCode(), Matchers.is(403));
    ...
    public void testWaitAuthHook_sendGoodAuthToken() throws Exception {
        ...
        // TODO: why do I have sometimes a code 202 instead of the expected 200
        assertThat(
                "Triggering the webhook without authentication should fail",
    ...
    public void testWaitAuthHook_sendIncorrectAuthToken() throws Exception {
        ...
        // thrown, but nothing was thrown"
        assertThat(
                "Triggering the webhook with wrong authentication should return a HTTP forbidden error",
                ex.getStatusCode(),
                Matchers.is(403));
    ...
    public void testLargeDataMessage() throws Exception {
        ...
        String pipelineCode = "node {  \n" + "   def hook = registerWebhook(token: \""
                + webHook_ID + "\")  \n" + "   echo \"token=${hook.token}\"  \n"
     ...
     * @return the webResponse from the webhook
     */
    public WebResponse trigger_authenticated_webhook(String webhook_path, String content, String authToken)
            throws IOException {
        JenkinsRule.WebClient wc = j.createWebClient();
        URL URLtoCall = new URL(j.getURL(), webhook_path);
        wc.addRequestHeader("content-type", "application/json");
